...
class IndentController extends Controller
{
    ...
    public function indexIndent()
    {
        ...
        if (auth()->user()->role_id === 1 || auth()->user()->role_id === 2) {
            ...
            $indents=$allindents->paginate(10);
            
        } elseif (auth()->user()->role_id === 3) { 
            $user = auth()->user();
            // Initialize query builder
            $allindents = Indent::with('indentRate')
                                ->where('user_id', $user->id)
                                ->where('is_follow_up', 0)
                                ->orderBy('id', 'desc');
            // If the user's role is 3 (assuming role_id 3 is for specific users)
            if ($user->role_id === 3) {
                $allindents->whereDoesntHave('indentRate');
            }
            // Paginate the results
            $indents = $allindents->paginate(15);
           

        } elseif (auth()->user()->role_id === 4) {
            $user = Auth::user();

    // Get indents and filter in the query
    $allIndents = Indent::with('indentRate')->where('is_follow_up', 0)->orderBy('id', 'desc');

    if ($user->role_id === 4) {
        $allIndents->whereDoesntHave('indentRate', function ($q) use ($user) {
            $q->where('user_id', $user->id);
        });
    }

    $indents = $allIndents->paginate(15); 
            // $allIndents = Indent::with('indentRate')->where('is_follow_up', 0)->orderBy('id', 'desc')->paginate(1);
            ...
            $indents = Indent::with('indentRate')->paginate(15);
        }
        ...
        return view('indent.index', compact('locations', 'indents', 'indentCount', 'selectedIndentId', 'weightUnits', 'materialTypes', 'truckTypes', 'quotedIndents', 'confirmedIndents', 'canceledIndents', 'followupIndents', 'salesPerson'));
    }   
    ...
    public function storeIndent(Request $request)
    {
        ...
        //echo 'asa<pre>'; print_r($indentData); exit;
        //echo 'sdsds<pre>'; print_r($indentData); exit;
        //Changes by Thamayanthi
    ...
    public function quoted()
    {
        ...
        if ($user->role_id === 1 || $user->role_id === 2) {
            ...
            //     foreach ($forwardRates as $key =>$rate) {
            //         $locationIdentifier = $rate->indent->pickup_location_id . '-' . $rate->indent->drop_location_id;
            //             $leastRates[$key] = $rate->indent_id;
            //             $secondLeastRate = Rate::orderBy('rate', 'asc')->where('indent_id', $rate->indent_id)->skip(1)->take(1)->first();
            ...
            //     foreach ($reverseRates as $key => $rate) {
            //         $locationIdentifier = $rate->indent->pickup_location_id . '-' . $rate->indent->drop_location_id;
            ...
            //         : null;
            // }
            // foreach($leastRate as $key => $value) {
            //     $leastRates[$key] = $value->indent_id;
            // }
        ...
        if (auth()->user()->role_id === 3) {
            ...
            $unquotedIndents = Indent::with('indentRate')->get();
        }
        ...
        return view('quoted', compact('materialTypes', 'truckTypes', 'leastRates', 'weightUnits', 'locations', 'indentCount', 'indents', 'secondLeastRateAmounts', 'selectedIndentId', 'user', 'confirmedLocations', 'confirmedIndents', 'canceledIndents', 'unquotedIndents', 'followupIndents', 'salesPerson'));
    ...
    public function confirmedLocations()
    {
        ...
        if (auth()->user()->role_id === 1 || auth()->user()->role_id === 2) {
            $indents = Indent::whereIn('status', [1, 2, 3, 4, 5])->with('cancelReasons')->orderBy('id', 'desc')->paginate(5);
        } elseif(auth()->user()->role_id === 3) {
            // Fetch indents belonging to the current user with cancel reasons
            ...
            })->with('indentRate')->latest()->orderBy('id', 'desc')->paginate(5);
        ...
        return view('indent.confirmed_locations', compact('indents', 'cancelReasons', 'canceledIndents', 'quotedIndents', 'unquotedIndents', 'followupIndents'));
    }
    ...
    public function getCanceledIndents()
    {
        ...
        if ($user->role_id === 1 || $user->role_id === 2) {
            $canceledIndents = Indent::onlyTrashed()->with('cancelReasons')->orderBy('id', 'desc')->paginate(15);
        } elseif ($user->role_id === 3) {
            // Fetch canceled indents belonging to the current user if user is sales representative
            $canceledIndents = Indent::where('user_id', $user->id)
                ->onlyTrashed()
        ...
        if (auth()->user()->role_id === 4) {
            ...
            $unquotedIndents = Indent::with('indentRate')->orderBy('id', 'desc')->get();
        }
        ...
        // dd($canceledIndents);
        return view('indent.canceled-indents', compact('canceledIndents', 'quotedIndents', 'unquotedIndents', 'confirmedIndents', 'followupIndents'));
    }
    ...
    public function getFollowupIndents()
    {
        ...
        if ($user->role_id === 1 || $user->role_id === 2) {
            $followupIndents = Indent::where('is_follow_up', 1)->orderBy('id', 'desc')->paginate(5);
        ...
        // dd($canceledIndents);
        return view('indent.followup', compact('canceledIndents', 'quotedIndents', 'unquotedIndents', 'confirmedIndents', 'followupIndents', 'indents'));
    }
    ...
    public function getCustomerDetails(Request $request) {
        ...
        try {
            $customer = Indent::where('number_1', $contactNumber)->firstOrFail();
