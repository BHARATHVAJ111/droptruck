...
<script src="https://cdn.jsdelivr.net/npm/axios/dist/axios.min.js"></script>
  ...
  <div class="card shadow border" style="margin:5% 20% 0 20%">
    ...
    <div class="card-body text-center">
      <p class="card-text"><strong class="label-width">Pickup Location:</strong><span>{{ ($indent->pickupLocation) ? $indent->pickupLocation->district : 'N/A' }}</span></p>
      <p class="card-text"><strong class="label-width">Drop Location:</strong><span> {{ ($indent->dropLocation) ? $indent->dropLocation->district : 'N/A' }}</span></p>
    ...
    @endif</span> --}}
        ...
        @if($indent->driver_rate != '0.00')
          ...
          @php
          $rateIds = $indent->indentRate->where('rate', $indent->driver_rate)->first();
          $rateId = ($rateIds) ? $rateIds->id : '';
          @endphp
          <input type="hidden" name="driver-rates" id="driver-rates" value="{{ $rateId }}">
        @else
           <select class="driver-rates" id="driver-rates">
                <option value="">Select Rates</option>
               @foreach($indentRates as $rate)
                <option value="{{ $rate->id }}"> {{ $rate->rate }} - {{ $rate->user->name }} - '<a href="#">X</a>'</option>
            @endforeach
  ...
  Customer Rate
      ...
      <a href="#" onclick="confirmToTrips('{{ $indent->id }}', $('#driver-rates').val()); return false;" class="btn btn-success" style="font-size: 8px; padding: 5px 10px;">Win</a>
